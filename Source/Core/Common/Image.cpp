...
{
  std::vector<u8>* stuff = (std::vector<u8>*)(png_get_io_ptr(png));
...
bool load_png(std::vector<u8> stuff, Image& image)
{
  png_struct* png;
  png_info* info;
  png_byte* data = NULL;
  ...
  png_set_read_fn(png, &stuff, read_func);
  ...
  stride = width * 4;
  data = reinterpret_cast<u8*>(malloc(stride * height));
  if (!data)
  {
    png_destroy_read_struct(&png, &info, NULL);
    return false;
  ...
  memcpy(image.data.data(), data, stride * height);
  image.width = width;
  image.height = height;
  return true;
}
